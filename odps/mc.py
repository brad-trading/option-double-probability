...
from scipy.stats import norm  # import once at module scope
...
class MonteCarlo:
    ...
    def doubling_probability(self, S0: float, K: float, T: float, r: float,
        ...
        for _ in range(num_batches):
            ...
            # for t = N-1 .. 0 (pricing is stateless; order doesn’t matter)
            for t in range(N):
                Ttm = Ttm_grid[t]
                sigma = sig_t[t]
                St = S[:, t]
                    ...
                    if np.any(quad):
                        ...
                        if np.any(pos):
                            ...
                            roots = np.vstack(((-bq[quad][pos] - sd) / a2,
                                               (-bq[quad][pos] + sd) / a2)).T
                            rts = np.where((roots > 0) & (roots < 1), roots, np.nan)
                            alpha_sel = np.nanmin(rts, axis=1)
